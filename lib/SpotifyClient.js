  ...
  (0, _createClass2["default"])(SpotifyClient, [{
    ...
    value: async function getRecentlyPlayedTracks() {
      ...
      var getNextPage = function getNextPage() {
        if (!next) return null;
        return _this3.getRecentlyPlayedTracks(next);
      };
    ...
    value: async function listPlaylists() {
      ...
      var getPage = function getPage(pageNumber) {
        return _this4.listPlaylists(pageNumber, limit);
      };

      var getNextPage = function getNextPage() {
        if (!next) return null;
        return _this4.listPlaylists(page + 1, limit);
      };
    ...
    value: function startPlaybackStateChangeListener() {
      ...
      this.playbackStateChangeListener = setInterval(async function () {
        await _this5.refreshAccessToken();
        var playbackData = await _this5.getCurrentTrack();
        if (!playbackData) return;

        if (!playbackData.item) {
          if (_this5.currentTrack) _this5.lastTrack = _this5.currentTrack;
          _this5.currentTrack = null;
          return;
        }

        if (!_this5.currentTrack) {
          _this5.currentTrack = playbackData;

          _this5.emit('newTrack', _this5.currentTrack);
        }

        if (playbackData.item.name !== _this5.currentTrack.item.name) {
          _this5.lastTrack = _this5.currentTrack;
          _this5.currentTrack = playbackData;
          if (_this5.lastTrack.progressMs <= _this5.lastTrack.item.durationMs - 5000) _this5.emit('trackSkipped', _this5.lastTrack);

          _this5.emit('newTrack', _this5.currentTrack);
        }

        if (playbackData.item.name === _this5.currentTrack.item.name) _this5.currentTrack = playbackData;
      }, 1000);
    }
  }]);
